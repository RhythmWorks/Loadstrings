...
local function _sameTeam(player)
    local lPlrTransparency = game.Players.LocalPlayer.Character.Shading.OutlineTransparency
    local plrTransparency = player.Character.Shading.OutlineTransparency

    return lPlrTransparency == plrTransparency
end

local function _getColor(player)
    return player.Character.Shading.OutlineTransparency == 1 and Color3.new(1, 1, 1) or player.Character.Shading.OutlineColor
end
		...
		local connection = game:GetService("RunService").RenderStepped:Connect(function()
			if player.Character and player.Character:FindFirstChild ("Humanoid") and player.Character:FindFirstChild ("HumanoidRootPart") and player ~= game.Players.LocalPlayer and player.Character.Humanoid.Health > 0 then
				...
				if onScreen then
					...
					Box.Visible = true
					Box.Color = _getColor(player) --player.TeamColor.Color or Color3.new(1, 0, 0)
...
local function Nametags()
	local function create(player)
		...
		username.TextColor3 = _getColor(player) --player.TeamColor.Color or Color3.new(1, 1, 1)
	...
	local function onUpdate()
		if aimbotEnabled then
			...
			if targetCharacter then
				getgenv().__0RXPT.TargetTeamColor = _getColor(game.Players[targetCharacter.Name]) --game.Players[targetCharacter.Name].Team.TeamColor.Color or Color3.new(1, 1, 1)
