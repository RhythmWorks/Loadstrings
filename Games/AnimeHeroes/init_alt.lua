...
local function destroyCXT()
	if getgenv().CXT then
		for _, connection in getgenv().CXT.Connections do
			connection:Disconnect()
		end

		for _, thread in getgenv().CXT.Threads do
			task.cancel(thread)
		end

		if getgenv().CXT.Window then
			getgenv().CXT.Window:Destroy()
		end
	end
end
...
-- do
-- 	local function _end()
-- 		if game.Players.LocalPlayer.Name ~= "0rxpt" then
-- 			game.Players.LocalPlayer:Kick("How did we get here?")
-- 		end
-- 	end

-- 	if not getgenv()._CXT then
-- 		return _end()
-- 	end

-- 	local key = getgenv()._CXT.Key
...
-- 				Url = "http://78.108.218.88:25170/key/" .. key,
-- 				Method = "GET",
-- 			})
...
local validWorldEnemies = {
	...
	["Slayer Village"] = { "Akaze", "Igure", "Hantenge", "Gyutare", "Rue" },
	["Marine Island"] = { "Peroni", "Magellon", "Akaine", "Aokije", "Kizare", "Hace" },
	["Leaf Village"] = { "Madare", "Tobe", "Minate", "Shikamare", "Gaare" },
}

do
	local t1 = task.defer(function()
		while task.wait() do
			getgenv().CXT = getgenv().CXT
				or CXT
		end
	end)
...
-- 	local btn = game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Container.Buttons.Auto.Button
-- 	local stopBtn = game.Players.LocalPlayer.PlayerGui.UI.Star.Stop.Button

-- 	local fastEgg = false

-- 	local t1 = task.defer(function()
-- 		while task.wait() do
-- 			if getgenv().CXT.AutoEgg then
-- 				if not fastEgg then
-- 					if game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Visible and not game.Players.LocalPlayer.PlayerGui.UI.Star.Visible then
-- 						if game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Container.FastOpen.OnOff.Text == "OFF" then
-- 							--local tries = 0
			
-- 							-- repeat
-- 							-- 	tries += 1
			
-- 							-- 	VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 175, btn.AbsolutePosition.Y + 75, 0, true, game, 1)
-- 							-- 	VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 175, btn.AbsolutePosition.Y + 75, 0, false, game, 1)
				
-- 							-- 	task.wait(0.5)
-- 							-- until game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Container.FastOpen.OnOff.Text == "ON" or tries >= 5
-- 						end
						
-- 						VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 50, btn.AbsolutePosition.Y + 80, 0, true, game, 1)
-- 						VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 50, btn.AbsolutePosition.Y + 80, 0, false, game, 1)
...
-- 						workspace.Client.Maps[workspace.Client.Maps:GetChildren()[1].Name].Stars.Normal.HumanoidRootPart.Position
-- 					)
-- 					task.wait(0.35)
-- 					game:GetService("ReplicatedStorage")
-- 						:WaitForChild("Bridge")
-- 						:FireServer("Stars", "Multi", workspace.Client.Maps:GetChildren()[1].Name)

-- 					repeat
-- 						task.wait()
-- 					until game.Players.LocalPlayer.PlayerGui.UI.Star.Visible and game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Visible

-- 					while game.Players.LocalPlayer.PlayerGui.UI.Star.Visible and game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Visible do
-- 						VirtualInputManager:SendMouseButtonEvent(0.5, 0.5, 0, true, game, 1)
-- 						VirtualInputManager:SendMouseButtonEvent(0.5, 0.5, 0, false, game, 1)
-- 						task.wait()
-- 					end

-- 					task.wait(0.35)
-- 					game.Players.LocalPlayer.Character:MoveTo(oldPosition)
-- 				end
-- 			elseif getgenv().CXT.FastEgg then
-- 				if game.Players.LocalPlayer.PlayerGui.UI.Star.Visible then
-- 					VirtualInputManager:SendMouseButtonEvent(stopBtn.AbsolutePosition.X + 150, stopBtn.AbsolutePosition.Y + 60, 0, true, game, 1)
-- 					VirtualInputManager:SendMouseButtonEvent(stopBtn.AbsolutePosition.X + 150, stopBtn.AbsolutePosition.Y + 60, 0, false, game, 1)
-- 				elseif game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Visible and not game.Players.LocalPlayer.PlayerGui.UI.Star.Visible then
-- 					VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 50, btn.AbsolutePosition.Y + 80, 0, true, game, 1)
-- 					VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 50, btn.AbsolutePosition.Y + 80, 0, false, game, 1)
-- 				end
-- 			else
...
-- do
-- 	local VirtualInputManager = game:GetService("VirtualInputManager")
-- 	local btn = game.Players.LocalPlayer.PlayerGui.UI.HUD.Star.Container.FastOpen

-- 	local t1 = task.defer(function()
-- 		while task.wait() do
-- 			if btn.OnOff.Text == "OFF" and btn.Parent.Parent.Visible then
-- 				repeat
-- 					VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 175, btn.AbsolutePosition.Y + 75, 0, true, game, 1)
-- 					VirtualInputManager:SendMouseButtonEvent(btn.AbsolutePosition.X + 175, btn.AbsolutePosition.Y + 75, 0, false, game, 1)
	
-- 					task.wait(0.5)
-- 				until btn.OnOff.Text == "ON"
-- 			end
-- 		end
-- 	end)
...
do
	local t1 = task.defer(function()
		while task.wait(3) do
			if getgenv().CXT.AutoAscend then
				game:GetService("ReplicatedStorage"):WaitForChild("Bridge"):FireServer("Ascension", "Evolve")
			end
		end
	end)
...
do
	local t1 = task.defer(function()
		while task.wait(3) do
			if getgenv().CXT.AutoAura then
				game:GetService("ReplicatedStorage"):WaitForChild("Bridge"):FireServer("Auras", "Evolve")
			end
		end
	end)
...
do
	local t1 = task.defer(function()
		while task.wait(3) do
			if getgenv().CXT.AutoBuyFruit then
				game:GetService("ReplicatedStorage"):WaitForChild("Bridge"):FireServer("FruitMerchant", "Buy", "Magma Fruit")
			end
		end
	end)
...
-- Handle AutoEquipBest
do
	local t1 = task.defer(function()
		while task.wait(3) do
			if getgenv().CXT.AutoEquipBest then
...
do
	local World = workspace.Client.Enemies.World
	local enemies = {}

	local function addEnemy(obj)
		repeat
			task.wait()
		until (getgenv().CXT.AutoFarmSelected.Farming and not getgenv().CXT.AutoFarmGamemode.InGamemode)
			or not obj:IsDescendantOf(World)

		if
			not getgenv().CXT.AutoFarmSelected.Farming
			or getgenv().CXT.AutoFarmGamemode.InGamemode
			or not obj:IsDescendantOf(World)
		then
			return
		end

		local selectedEnemies = getgenv().CXT.AutoFarmSelected.Enemies

		if #selectedEnemies > 0 then
			if not table.find(selectedEnemies, obj.HumanoidRootPart.HUD.Frame.Title.Text) then
				return
			end
		end

		if table.find(enemies, obj) then
			return
		end
	...
	for _, obj in World:GetChildren() do
		local t = task.defer(function()
			while task.wait() do
				if not obj:IsDescendantOf(World) then
					return
				end
		...
		table.insert(getgenv().CXT.Threads, t)
	end

	local c1 = World.ChildAdded:Connect(addEnemy)
	table.insert(getgenv().CXT.Connections, c1)

	local function findNextEnemy()
		local selectedEnemies = getgenv().CXT.AutoFarmSelected.Enemies
		for _, obj in World:GetChildren() do
			if obj:IsDescendantOf(World) and table.find(selectedEnemies, obj.HumanoidRootPart.HUD.Frame.Title.Text) then
				return obj
			end
		end
		return nil
	...
	local function shouldTeleport(worldName)
		local selectedEnemies = getgenv().CXT.AutoFarmSelected.Enemies
		local worldEnemies = validWorldEnemies[worldName] or {}
		for _, enemy in ipairs(selectedEnemies) do
			if table.find(worldEnemies, enemy) then
				return true
			end
		end
		return false
	...
	local t1 = task.defer(function()
		while task.wait() do
			if getgenv().CXT.AutoFarmSelected.Farming and not getgenv().CXT.AutoFarmGamemode.InGamemode then
				local currentWorld = workspace.Client.Maps:GetChildren()[1].Name
				local selectedWorlds = getgenv().CXT.AutoFarmSelected.Worlds

				for _, world in selectedWorlds do
					if shouldTeleport(world) and currentWorld ~= world then
						game:GetService("ReplicatedStorage")
							:WaitForChild("Bridge")
							:FireServer("Teleport", "Teleport", world)
						task.wait(0.5)

						if workspace.Client.Maps:GetChildren()[1].Name == world then
							break
						end
					end
				end
				...
				if nextEnemy then
					game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = nextEnemy.HumanoidRootPart.CFrame
						* CFrame.new(0, 0, 2)
					while getgenv().CXT.AutoFarmSelected.Farming and nextEnemy:IsDescendantOf(World) do
						if
							not table.find(
								getgenv().CXT.AutoFarmSelected.Enemies,
								nextEnemy.HumanoidRootPart.HUD.Frame.Title.Text
							)
						then
							break
						end
...
-- Handle AutoFarmGamemode
do
	local Gamemodes = workspace.Client.Enemies.Gamemodes
	local enemies = {}

	local function addEnemy(obj)
		repeat
			task.wait()
		until getgenv().CXT.AutoFarmGamemode.Farming or not obj:IsDescendantOf(Gamemodes)

		if not getgenv().CXT.AutoFarmGamemode.Farming or not obj:IsDescendantOf(Gamemodes) then
			return
		end
	...
	for _, obj in Gamemodes:GetChildren() do
		local t = task.defer(function()
			while task.wait() do
				if not obj:IsDescendantOf(Gamemodes) then
					return
				end
	...
	local c1 = Gamemodes.ChildAdded:Connect(addEnemy)
	table.insert(getgenv().CXT.Connections, c1)

	local t1 = task.defer(function()
		while task.wait() do
			if getgenv().CXT.AutoFarmGamemode.Farming and #getgenv().CXT.AutoFarmGamemode.Gamemode > 0 then
				local waveTxt =
					game.Players.LocalPlayer.PlayerGui.UI.HUD.Gamemodes[getgenv().CXT.AutoFarmGamemode.Gamemode].Frame.Frame.Title.UID.Text
				local wave = waveTxt:sub(6, #waveTxt)

				if getgenv().CXT.AutoFarmGamemode.InGamemode or #Gamemodes:GetChildren() > 0 then
					local index = 1
					local nextEnemies = Gamemodes:GetChildren()

					for key, enemy in nextEnemies do
						if
							not game:GetService("ReplicatedStorage").Enemies.Gamemodes[getgenv().CXT.AutoFarmGamemode.Gamemode]
								:FindFirstChild(enemy.HumanoidRootPart.HUD.Frame.Title.Text)
						then
							table.remove(nextEnemies, key)
						end
					end

					local nextEnemy = nextEnemies[index] --enemies[index]

					if not nextEnemy then
						continue
					end

					if not nextEnemy:IsDescendantOf(Gamemodes) then
						table.remove(enemies, index)
						continue
					end

					if
						nextEnemy.HumanoidRootPart.HUD.Frame.Difficult.Text == "Boss"
						and #Gamemodes:GetChildren() > 1
					then
						index = 2
						nextEnemy = Gamemodes:GetChildren()[index] --enemies[index]
					end
					...
					if tonumber(wave) >= getgenv().CXT.AutoFarmGamemode.ExitWave then
						game:GetService("ReplicatedStorage")
							:WaitForChild("Bridge")
							:FireServer("Teleport", "Teleport", "Leaf Village")
						task.wait(0.5)
						getgenv().CXT.AutoFarmGamemode.InGamemode = false

						task.delay(1, function()
							getgenv().CXT.AutoFarmGamemode.Farming = false

							task.wait(2)
							getgenv().CXT.AutoFarmGamemode.Farming = true
						end)
					...
					if
						not game:GetService("ReplicatedStorage").Enemies.Gamemodes[getgenv().CXT.AutoFarmGamemode.Gamemode]
							:FindFirstChild(nextEnemy.HumanoidRootPart.HUD.Frame.Title.Text)
					then
						continue
					end

					getgenv().CXT.AutoFarmGamemode.InGamemode = true

					repeat
						if not getgenv().CXT.AutoFarmGamemode.Farming then
							break
						end
						...
						if nextEnemy.HumanoidRootPart then
							game.Players.LocalPlayer.Character:MoveTo(nextEnemy.HumanoidRootPart.Position)
							--game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = nextEnemy.HumanoidRootPart.CFrame * CFrame.new(0, 0, 2)
						end
						
						task.wait()
					until not getgenv().CXT.AutoFarmGamemode.Farming
						or not nextEnemy:IsDescendantOf(Gamemodes)
						or tonumber(wave) >= getgenv().CXT.AutoFarmGamemode.ExitWave
						or not game:GetService("ReplicatedStorage").Enemies.Gamemodes[getgenv().CXT.AutoFarmGamemode.Gamemode]
							:FindFirstChild(nextEnemy.HumanoidRootPart.HUD.Frame.Title.Text)
						or nextEnemy.HumanoidRootPart.HUD.Frame.Difficult.Text == "Boss"
							and #Gamemodes:GetChildren() > 1
					...
					game:GetService("ReplicatedStorage")
						:WaitForChild("Bridge")
						:FireServer("Gamemodes_" .. getgenv().CXT.AutoFarmGamemode.Gamemode, "Start", "Public")
					--game:GetService("ReplicatedStorage"):WaitForChild("Bridge"):FireServer("Gamemodes_" .. getgenv().CXT.AutoFarmGamemode.Gamemode, "Join")
				end
			else
				getgenv().CXT.AutoFarmGamemode.InGamemode = false
			end
...
local Window = Fluent:CreateWindow({
	Title = "Anime Heroes | CXT Hub",
	SubTitle = "by RhythmWorks",
	TabWidth = 160,
...
getgenv().CXT.Window = Window

local Tabs = {
	Player = Window:AddTab({ Title = "Player", Icon = "hand" }),
	AutoMain = Window:AddTab({ Title = "Auto Main", Icon = "swords" }),
	AutoGamemodes = Window:AddTab({ Title = "Auto Gamemode", Icon = "gamepad" }),
	Teleports = Window:AddTab({ Title = "Teleports", Icon = "globe" }),
	Settings = Window:AddTab({ Title = "Settings", Icon = "settings" }),
	Credits = Window:AddTab({ Title = "Credits", Icon = "hammer" })
}
...
-- HideAlerts
do
	local Toggle = Tabs.Player:AddToggle("HideAlerts", { Title = "Hide Alerts", Default = false })

	Toggle:OnChanged(function()
		game.Players.LocalPlayer.PlayerGui.UI.Notifications.Visible = not Options.HideAlerts.Value
	end)

	Options.HideAlerts:SetValue(false)
end

-- AutoClick
-- do
-- 	local Toggle = Tabs.AutoMain:AddToggle("AutoClick", { Title = "Auto Click", Default = false })

-- 	local VirtualInputManager = game:GetService("VirtualInputManager")
-- 	local imageButton: ImageButton = game.Players.LocalPlayer.PlayerGui.UI.HUD.Bottom.Buttons.Bottom.Background

-- 	Toggle:OnChanged(function()
-- 		if Options.AutoClick.Value then
-- 			while imageButton.Image:match("15628157151") do
-- 				VirtualInputManager:SendMouseButtonEvent(
-- 					imageButton.AbsolutePosition.X + 35,
-- 					imageButton.AbsolutePosition.Y + 65,
-- 					0,
-- 					true,
-- 					game,
-- 					1
...
-- 			end
-- 		else
-- 			while imageButton.Image:match("15433949391") do
-- 				VirtualInputManager:SendMouseButtonEvent(
-- 					imageButton.AbsolutePosition.X + 35,
-- 					imageButton.AbsolutePosition.Y + 65,
-- 					0,
...
-- 	Options.AutoClick:SetValue(true)
-- end

-- AutoEgg
do
	local Toggle = Tabs.AutoMain:AddToggle("AutoEgg", { Title = "Auto Egg", Description = "Auto open eggs for current world.", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoEgg = Options.AutoEgg.Value
	end)
...
-- FastEgg
do
	local Toggle = Tabs.AutoMain:AddToggle("FastEgg", { Title = "Fast Auto Egg", Description = "Fast auto egg open for current world.", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.FastEgg = Options.FastEgg.Value
	end)

	Options.FastEgg:SetValue(false)
end

-- AutoAscend
do
	local Toggle = Tabs.AutoMain:AddToggle("AutoAscend", { Title = "Auto Ascend", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoAscend = Options.AutoAscend.Value
	end)

	Options.AutoAscend:SetValue(false)
end

-- AutoAura
do
	local Toggle = Tabs.AutoMain:AddToggle("AutoAura", { Title = "Auto Evolve Aura", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoAura = Options.AutoAura.Value
	end)

	Options.AutoAura:SetValue(false)
end

-- AutoBuyFruit
do
	local Toggle = Tabs.AutoMain:AddToggle("AutoBuyFruit", { Title = "Auto Buy Fruit", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoBuyFruit = Options.AutoBuyFruit.Value
	end)

	Options.AutoBuyFruit:SetValue(false)
end

-- AutoEquipBest
do
	local Toggle = Tabs.AutoMain:AddToggle("AutoEquipBest", { Title = "Auto Equip Best", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoEquipBest = Options.AutoEquipBest.Value
	end)
...
-- AutoFarmSelected
do
	local Toggle = Tabs.AutoMain:AddToggle("AutoFarmSelected", { Title = "Auto Farm Selected", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoFarmSelected.Farming = Options.AutoFarmSelected.Value
	end)
end

-- AutoFarmWorlds + AutoFarmEnemies
do
	local Dropdown = Tabs.AutoMain:AddDropdown("AutoFarmWorlds", {
	...
	local Dropdown2 = Tabs.AutoMain:AddDropdown("AutoFarmEnemies", {
	...
	local function updateWorldEnemies()
		local worldEnemies = {}

		for _, world in getgenv().CXT.AutoFarmSelected.Worlds do
			for _, enemy in validWorldEnemies[world] do
				table.insert(worldEnemies, enemy)
			end
		end

		local currentSelectedEnemies = Options.AutoFarmEnemies.Value
		local filteredEnemies = {}

		for enemy, _ in next, currentSelectedEnemies do
			if table.find(worldEnemies, enemy) then
				filteredEnemies[enemy] = true
			end
		end

		Dropdown2:SetValues(worldEnemies)
		Dropdown2:SetValue(filteredEnemies)
	end

	Dropdown:OnChanged(function(value)
		local worlds = {}

		for world, _ in next, value do
			table.insert(worlds, world)
		end

		getgenv().CXT.AutoFarmSelected.Worlds = worlds

		if not updateWorldEnemies then
			repeat
				task.wait()
			until updateWorldEnemies
		end
	...
	Dropdown2:OnChanged(function(value)
		...
		for enemy, _ in next, value do
			table.insert(enemies, enemy)
		end

		getgenv().CXT.AutoFarmSelected.Enemies = enemies
	end)
end

-- AutoFarmGamemode
do
	local Toggle = Tabs.AutoGamemodes:AddToggle("AutoFarmGamemode", { Title = "Auto Farm Gamemode", Default = false })

	Toggle:OnChanged(function()
		getgenv().CXT.AutoFarmGamemode.Farming = Options.AutoFarmGamemode.Value
	end)
end
...
do
	local Dropdown = Tabs.AutoGamemodes:AddDropdown("Gamemodes", {
		Title = "Gamemodes",
		Values = { "Bizarre Defense", "Dragon Tournament", "Ghost Train", "Treasure Hunt", "Ninja Tournament" },
		Multi = false,
	})

	Dropdown:OnChanged(function(value)
		if not value then
			getgenv().CXT.AutoFarmGamemode.Gamemode = ""
			return
		end

		getgenv().CXT.AutoFarmGamemode.Gamemode = value:gsub("%s+", "")
	end)
end

-- Exit On Wave
do
	local Slider = Tabs.AutoGamemodes:AddSlider("ExitOnWave", {
		Title = "Exit on Wave",
		Description = "Exit the gamemode at this wave",
		Default = 100,
		Min = 2,
		Max = 100,
		Rounding = 0,
		Callback = function(value)
			getgenv().CXT.AutoFarmGamemode.ExitWave = value
		end,
	})
...
do
	...
	Dropdown:OnChanged(function(value)
		game:GetService("ReplicatedStorage"):WaitForChild("Bridge"):FireServer("Teleport", "Teleport", value)
	end)
end

-- Player Teleports
do
	local Dropdown = Tabs.Teleports:AddDropdown("Players", {
		Title = "TP to Player",
		Values = {},
		Multi = false,
	})
	...
	local function updatePlayers()
		...
		for _, player in game.Players:GetPlayers() do
			...
			table.insert(
				players,
				if player.Name == player.DisplayName then player.Name else `{player.DisplayName} (@{player.Name})`
			)
		end
	...
	local c1 = game.Players.PlayerAdded:Connect(updatePlayers)
	local c2 = game.Players.PlayerRemoving:Connect(updatePlayers)

	table.insert(getgenv().CXT.Connections, c1)
	table.insert(getgenv().CXT.Connections, c2)
	...
	Dropdown:OnChanged(function(value)
		...
		if string.find(value, "@") then
			local playerName = string.sub(value, string.find(value, "@") + 1, string.find(value, ")") - 1)
			game.Players.LocalPlayer.Character:MoveTo(game.Players[playerName].Character.HumanoidRootPart.Position)
		else
			game.Players.LocalPlayer.Character:MoveTo(game.Players[value].Character.HumanoidRootPart.Position)
		end
	end)
end
...
do
	Tabs.Credits:AddParagraph({
        Title = "Creator",
        Content = "This script hub was developed by RhythmWorks."
    })
	...
	Tabs.Credits:AddButton({
        ...
        Callback = function()
            setclipboard("https://discord.gg/AwbNteDxNd")
...
SaveManager:SetFolder("CXTHub/AnimeHeroes")
...
Fluent:Notify({
	Title = "CXT Hub",
	Content = "Do NOT use AutoFarmSelected and AutoFarmGamemode at the same time (it bugs)!",
	Duration = 8,
})